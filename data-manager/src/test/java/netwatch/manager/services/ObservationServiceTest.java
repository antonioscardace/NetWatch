...
class ObservationServiceTest {
    ...
    void testInsertObservation_Successful() {
        when(observationRepository.existsByObservationIdAddressAndObservationIdContact(utility.getAddress(), contact.getValue())).thenReturn(false);
        when(utilityRepository.existsByAddress(utility.getAddress())).thenReturn(true);
        when(contactRepository.existsByValue(contact.getValue())).thenReturn(true);
    ...
    void testInsertObservation_Unsuccessful(boolean observationExists, boolean utilityExists, boolean contactExists) {
        when(observationRepository.existsByObservationIdAddressAndObservationIdContact(utility.getAddress(), contact.getValue())).thenReturn(observationExists);
        when(utilityRepository.existsByAddress(utility.getAddress())).thenReturn(utilityExists);
        when(contactRepository.existsByValue(contact.getValue())).thenReturn(contactExists);
    ...
    void testDeleteObservation_Unsuccessful(boolean observationExists) {
        when(observationService.checkIfObservationConcernsEnvironment(utility.getAddress(), contact.getValue(), "admin")).thenReturn(false);
        when(observationService.checkIfReferentObservesUtility(utility.getAddress(), contact.getValue())).thenReturn(observationExists);
        when(observationRepository.findByObservationIdAddressAndObservationIdContact(utility.getAddress(), contact.getValue())).thenReturn(observation);
